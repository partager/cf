...
let mediaScanUrl = 'assets/js/vv/scan.php';
...
class MediaModal {
	constructor (modal = true)
	{
		...
		this.fileList = null;
		this.mediaPath = "assets/js/vv/media";
		this.install_url = document.getElementById("sfeditor_install_url").value;
		this.type = "single";
	}
	...
	save() {
		...
		if (file.indexOf("//") == -1) {
			file = 'assets/js/vv/media' + file.replace(" ","_");
		}
	...
	open(element, callback) {
		if (element instanceof Element) {
			this.targetInput = element.dataset.bsTargetInput;
			this.targetThumb = element.dataset.bsTargetThumb;
			if (element.dataset.type) {
				this.type = element.dataset.type;
			}
		} else if (element) {
			this.targetInput = element.bsTargetInput;
			this.targetThumb = element.bsTargetThumb;
			if (element.type) {
...
_
		...
		onUpload(event) {
			...
			function imageIsLoaded(e) {
					...
					$.ajax({
						type: "POST",
						url: 'assets/js/vv/upload.php',//set your server side upload script url
						data: formData,
						processData: false,
						contentType: false,
		...
		addFile(f, selected) {
				...
				var fileSize = _this.bytesToSize(f.size),
					...
					if (fileType == "jpg" || fileType == "jpeg" || fileType == "png" || fileType == "gif" || fileType == "svg" || fileType == "webp") {
						...
						icon = '<img class="image" loading="lazy" src="assets/js/vv/media' + f.path + '">';
					} else {
						...
						<label class="form-check">\
						  ...
						  <div href="#\" class="files">'+icon+'<div class="info"><div class="name">'+ name +'</div><span class="details">'+fileSize+'</span>\
							<a href="javascript:void(0);" class="preview-link"><i class="la la-search-plus"></i></a>\
							 <div class="preview">\
								<img src="assets/js/vv/media' +f.path + '">\
								<div>\
