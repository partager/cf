...
require($current_base_dir . "/controller/basic.php");

require_once($current_base_dir . "/library/library.php");
$load = new Library();
$cf_product_code = "cloudfunnelspro";
...
<p>Thanks</p>');
			...
			if (!get_option('qfnl_has_update_466')) {
				add_option('qfnl_has_update_466', 1);
			}
			echo 1;
		} elseif ($register == 2) {
...
if (isset($_POST['admin_forgot_password'])) {
	...
	if (isset($_POST['email'])) {
		...
		if (isset($_SESSION['fpwd_step_done' . get_option('site_token')])) {
			if ($_SESSION['fpwd_step_done' . get_option('site_token')] != 1) {
				die("Unauthorized Access, please try
again.");
			}
		} else {
			die("Unauthorized Access, try again.");
		}
		$otp = $user_ob->fpwdOTPVerification($_POST['otp']);
		if ($otp == 1) {
			$_SESSION['fpwd_step_done' . get_option('site_token')] = 2;
			echo 1;
		} elseif ($otp == 2) {
			echo "Unauthorized attempt, Please Try Again";
		} elseif ($otp == 0) {
			echo "OTP did not match";
		}
	} elseif (isset($_POST['pass'])) {
		if (isset($_SESSION['fpwd_step_done' . get_option('site_token')])) {
			if ($_SESSION['fpwd_step_done' . get_option('site_token')] != 2) {
				die("Unauthorized
Access, please try again.");
		...
		if ($addpass == 1) {
			unset($_SESSION['fpwd_step_done' . get_option('site_token')]);
			echo 1;
		} elseif ($addpass == 2 || $addpass == 0) {
...
if (isset($_POST['checkforqfnl_update'])) {
	...
	if (!isset($_SESSION['qfnl_install_later' . get_option('site_token')])) {
		...
		if ($do == "check") {
			...
			global $current_app_version;
			echo $autoupdater->installDependencies($current_app_version);
...
//check user is logged in and update data
...
if (isset($_POST['compose_cf_mail']) && isset($_POST['type'])) {
	...
	if ($type == 'init') {
		$data = $composer->init($_POST['title'], $_POST['smtps'], $_POST['lists'], $_POST['custom_emails'], $_POST['sentdata'], $_POST['extra_setup'], $_POST['formData']);
		if ($data) {
			echo json_encode($data);
		} else {
			echo 0;
...
if (isset($_POST['manage_aiwriter'])) {
	$ai_ob = $load->loadAi();
	$prompt_option = $_POST['prompt_option'];
	$detailed_explanation = $_POST['detailed_explanation'];
	$response = $ai_ob->aiAPIResponse($prompt_option, $detailed_explanation);
	echo $response;
}
